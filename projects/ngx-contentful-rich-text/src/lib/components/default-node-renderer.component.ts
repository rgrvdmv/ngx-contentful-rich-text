...
@Component({
  template: `
    <ng-container [ngSwitch]="node.nodeType">
      ...
      <table *ngSwitchCase="BLOCKS.TABLE">${CHILDREN}</table>
      <tr *ngSwitchCase="BLOCKS.TABLE_ROW">${CHILDREN}</tr>
      <th *ngSwitchCase="BLOCKS.TABLE_HEADER_CELL">${CHILDREN}</th>
      <td *ngSwitchCase="BLOCKS.TABLE_CELL">${CHILDREN}</td>
